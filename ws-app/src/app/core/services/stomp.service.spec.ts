...
describe('StompService', () => {
  ...
  describe('Lifecycle Resilience', () => {
    it('should not crash when disconnect is called immediately after creation', () => {
      const newService = TestBed.inject(StompService);
      expect(() => {
  ...
  describe('Runtime Config Overrides', () => {
    it('should keep defaults when runtime config fetch fails', async () => {
      fetchSpy.and.rejectWith(new Error('network down'));

      const refreshSpy = spyOn<any>(service, 'refreshNetworkConfig').and.callThrough();
      const promise = (service as any).prefetchRuntimeSocketConfig();
      await promise;
